...
public class KafkaConsumerConfig {
    ...
    public ConcurrentKafkaListenerContainerFactory<String, JobExecutionEvent> jobExecutionListenerFactory(
        ...
        factory.setConcurrency(concurrency);
        factory.getContainerProperties().setAckMode(ContainerProperties.AckMode.MANUAL_IMMEDIATE);
        factory.setCommonErrorHandler(buildErrorHandler(jobExecutionKafkaTemplate));

        // FIX: consumeExecutionResults() takes List<JobExecutionEvent> — must enable
        // batch mode or Spring Kafka will try to deserialize the whole batch as a
        // single JobExecutionEvent and fail silently on every message.
        factory.setBatchListener(true);
